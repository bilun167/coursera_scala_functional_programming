...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.groupBy((ch: Char) => ch.toLower).mapValues(_.size).toList.sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    if (occurrences.isEmpty) List(List())
      ...
      val remOccs: List[Occurrences] = combinations(occurrences.tail)
      (for {
        i <- 0 until (occurrences.head._2 + 1)
        occs <- remOccs
      } yield if (i == 0) occs else (occurrences.head._1, i) :: occs).toList
    }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    if (y.isEmpty) x
      ...
      val (fst, rem) = x.span((p: (Char, Int)) => p._1 != y.head._1)
      val occ = rem.head._2 - y.head._2
      val newRem = if (occ > 0) (rem.head._1, occ) :: rem.tail else rem.tail
      subtract(fst:::newRem, y.tail)
    }
